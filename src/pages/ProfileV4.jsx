...
const colorOptions = [
  { id: "blue", label: "Blue", dot: "bg-blue-500", soft: "bg-blue-50 text-blue-700 border-blue-100" },
  { id: "green", label: "Green", dot: "bg-emerald-500", soft: "bg-emerald-50 text-emerald-700 border-emerald-100" },
  { id: "purple", label: "Purple", dot: "bg-violet-500", soft: "bg-violet-50 text-violet-700 border-violet-100" },
  { id: "orange", label: "Orange", dot: "bg-orange-500", soft: "bg-orange-50 text-orange-700 border-orange-100" },
  { id: "pink", label: "Pink", dot: "bg-pink-500", soft: "bg-pink-50 text-pink-700 border-pink-100" },
  { id: "red", label: "Red", dot: "bg-red-500", soft: "bg-red-50 text-red-700 border-red-100" },
  { id: "teal", label: "Teal", dot: "bg-teal-500", soft: "bg-teal-50 text-teal-700 border-teal-100" },
  { id: "slate", label: "Slate", dot: "bg-slate-500", soft: "bg-slate-50 text-slate-700 border-slate-100" },
];
...
function getMembers(profile, user, myEmail) {
  ...
  (profile?.members || []).forEach((member, index) => {
    add({
      ...
      color: member.color || member.familyColor || "green",
      admin: member.isAdmin === true || member.is_admin === true,
      locked: false,
      permissions: member.permissions || makeDefaultPermissions(),
    });
  });
...
function FamilyCard({ family, active, onSelect }) {
  ...
  const memberCount = getMemberEmails(family).length || (Array.isArray(family?.members) ? family.members.length + 1 : 1);
...
function MemberCard({ member, onEdit, onDelete }) {
  const color = colorMeta(member.color);
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            <span className={`rounded-full border px-2 py-0.5 text-[10px] font-black ${color.soft}`}>Family color: {color.label}</span>
          </div>
        </div>
      </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {parents.map((parent, index) => {
              const color = colorMeta(parent.color || (index === 0 ? "blue" : "orange"));
              return (
                <span key={`${parent.email || parent.name}-${index}`} className={`rounded-full border px-2 py-1 text-[10px] font-black ${color.soft}`}>
...
export default function ProfileV4() {
  ...
  const handleRemoveChild = (childToRemove) => {
    setConfirmAction({
      title: "Remove child?",
      description: `Remove ${childToRemove} from this family space?`,
      confirmText: "Remove",
      tone: "danger",
      onConfirm: async () => {
    ...
    try {
      ...
      if (memberEditor.source === "owner") {
        updates = {
          parent1Name: name,
          parent1_name: name,
          parent1Role: memberEditor.role,
          parent1_role: memberEditor.role,
          parent1Color: memberEditor.color,
          parent1_color: memberEditor.color,
        };
        ...
        updates = {
          ...
          parent2Email: email,
          parent2_email: email,
          parent2Role: memberEditor.role,
          parent2_role: memberEditor.role,
          parent2Color: memberEditor.color,
          parent2_color: memberEditor.color,
        };
      } else if (memberEditor.mode === "add") {
        updatedMembers.push({
          name,
          email,
          role: memberEditor.role,
          color: memberEditor.color,
          familyColor: memberEditor.color,
          isAdmin: memberEditor.admin === true,
          permissions: makeDefaultPermissions(),
        });
        updates = { members: updatedMembers };
      } else {
        updatedMembers = updatedMembers.map((member) =>
          normalizeEmail(member.email) === normalizeEmail(memberEditor.originalEmail)
            ? {
                ...member,
                name,
                email,
  ...
  const deleteMember = async (member) => {
    ...
    try {
      ...
      const memberEmails = getMemberEmails(profile).filter((item) => normalizeEmail(item) !== email);
  ...
  const openCustodyEditor = (group) => {
    ...
    setCustodyEditor({
      ...
      parentName: currentParents[0]?.name || parent1Name || user?.displayName || "Parent",
      parentEmail: currentParents[0]?.email || myEmail || "",
      parentRole: currentParents[0]?.role || parent1Role || "parent",
      parentColor: currentParents[0]?.color || "blue",
      coparentName: currentParents[1]?.name || "",
      coparentEmail: currentParents[1]?.email || "",
      coparentRole: currentParents[1]?.role || (parent1Role === "dad" ? "mom" : "dad"),
      coparentColor: currentParents[1]?.color || "orange",
    });
  };
  ...
  const saveCustodyEditor = async () => {
    ...
    try {
      const updatedParents = [
        {
          uid: user?.uid || null,
          email: normalizeEmail(custodyEditor.parentEmail) || myEmail || "",
          name: custodyEditor.parentName.trim() || "Parent",
          role: custodyEditor.parentRole || "parent",
          color: custodyEditor.parentColor,
          permissions: { custodyCalendar: { read: true, write: true } },
        },
        {
          uid: null,
          email: normalizeEmail(custodyEditor.coparentEmail),
          name: custodyEditor.coparentName.trim() || "Co-parent",
          role: custodyEditor.coparentRole || "parent",
          color: custodyEditor.coparentColor,
          permissions: { custodyCalendar: { read: true, write: true } },
        },
      ];
      ...
      await updateDoc(doc(db, "custodyGroups", custodyEditor.id), {
        ...
        children: [custodyEditor.child.trim()].filter(Boolean),
    ...
    <div className="min-h-screen bg-[#f8fbff] p-4 md:p-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-5 flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-500">Private family spaces, shared custody groups, members and colors.</p>
              <div className="mt-2 flex flex-wrap gap-2">
        ...
        <div className="mb-5 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
          {tabs.map((tab) => (
            <TabButton key={tab.id} tab={tab} active={activeTab === tab.id} onClick={() => selectTab(tab.id)} />
          ))}
        </div>
            ...
            <Card className="p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-400">Edit the private household space and family-specific colors.</p>
                </div>
          ...
          <div className="space-y-5">
            <Card className="border-blue-100 bg-blue-50 p-5">
              ...
              <p className="mt-1 text-sm font-semibold text-slate-500">
                Custody groups are shared co-parent spaces. A person can have a different color here than in a private family space.
              </p>
            </Card>
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  {!loadingCustody && custodyGroups.map((group) => (
                    <CustodyGroupCard
                      key={group.id}
                      group={group}
                      onEdit={openCustodyEditor}
                      onReset={(item) => setConfirmAction({
                        title: "Reset custody data?",
                        description: `This deletes custody schedule days for ${item.name || "this custody group"}, but keeps the group itself.`,
                        confirmText: "Reset data",
                        tone: "warning",
                        onConfirm: () => resetCustodyGroup(item),
                      })}
                      onDelete={(item) => setConfirmAction({
                        title: "Delete custody group?",
                        description: `Delete ${item.name || "this custody group"}? This removes the shared custody group.`,
                        confirmText: "Delete group",
                        tone: "danger",
                        showDeleteDaysOption: true,
                        deleteDays: false,
                        onConfirm: (deleteDays) => deleteCustodyGroup(item, deleteDays),
                      })}
                    />
                  ))}
              ...
              <Card className="p-5">
                ...
                {showCreateCustody ? (
                  <div className="space-y-3">
                    ...
                    <ColorPicker label="My custody color" value={custodyParentColor} onChange={setCustodyParentColor} />
                    <ColorPicker label="Co-parent custody color" value={custodyCoparentColor} onChange={setCustodyCoparentColor} />
        ...
        {activeTab === "members" && (
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_420px]">
            <Card className="p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider text-slate-500"><Shield className="h-4 w-4" /> Members & Permissions</h2>
                <Button
                  type="button"
                  onClick={() => setMemberEditor({ mode: "add", source: "member", name: "", email: "", role: "caregiver", color: "green", admin: false })}
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {members.map((member, index) => (
                  <MemberCard
                    ...
                    member={member}
                    onEdit={(item) => setMemberEditor({ ...item, mode: "edit", originalEmail: item.email })}
                    onDelete={(item) => setConfirmAction({
                      title: "Delete member?",
                      description: `Remove ${item.name || item.email || "this member"} from this family space?`,
                      confirmText: "Delete member",
                      tone: "danger",
                      onConfirm: () => deleteMember(item),
                    })}
                  />
                ))}
              </div>
            ...
            <Card className="p-5">
              <h2 className="mb-4 flex items-center gap-2 text-sm font-black uppercase tracking-wider text-slate-500"><Palette className="h-4 w-4" /> Color rules</h2>
              <div className="space-y-3 text-sm font-semibold text-slate-500">
                <p>Family colors are used in private household modules.</p>
      ...
      {memberEditor && (
        <Modal
          title={memberEditor.mode === "add" ? "Add family member" : "Edit family member"}
          description="Add grandparents, babysitters, caregivers, or family members. This is for the private family space."
          onClose={() => setMemberEditor(null)}
          footer={
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setMemberEditor(null)}>Cancel</Button>
              <Button type="button" onClick={saveMemberEditor} disabled={saving} className="bg-indigo-600 hover:bg-indigo-700">
        ...
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div><Label>Name</Label><Input value={memberEditor.name} onChange={(e) => setMemberEditor((current) => ({ ...current, name: e.target.value }))} className="mt-1" /></div>
            <div><Label>Email</Label><Input value={memberEditor.email} onChange={(e) => setMemberEditor((current) => ({ ...current, email: e.target.value }))} disabled={memberEditor.source === "owner"} className="mt-1" /></div>
            <div>
              <Label>Role</Label>
              <select value={memberEditor.role} onChange={(e) => setMemberEditor((current) => ({ ...current, role: e.target.value }))} className="mt-1 w-full rounded-xl border bg-white px-3 py-2">
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600">
                <input type="checkbox" checked={memberEditor.admin === true} onChange={(e) => setMemberEditor((current) => ({ ...current, admin: e.target.checked }))} disabled={memberEditor.source === "owner"} />
                Admin access
              </label>
            </div>
            <div className="md:col-span-2">
              <ColorPicker label="Family color" value={memberEditor.color} onChange={(color) => setMemberEditor((current) => ({ ...current, color }))} />
            </div>
          </div>
        </Modal>
      )}
      ...
      {custodyEditor && (
        <Modal
          title="Edit custody group"
          description="This is separate from the private family colors."
          onClose={() => setCustodyEditor(null)}
          footer={
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setCustodyEditor(null)}>Cancel</Button>
              <Button type="button" onClick={saveCustodyEditor} disabled={saving} className="bg-blue-600 hover:bg-blue-700">
                {saving ? "Saving..." : "Save custody group"}
              </Button>
            </div>
          }
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div><Label>Group name</Label><Input value={custodyEditor.name} onChange={(e) => setCustodyEditor((current) => ({ ...current, name: e.target.value }))} className="mt-1" /></div>
            <div><Label>Child</Label><Input value={custodyEditor.child} onChange={(e) => setCustodyEditor((current) => ({ ...current, child: e.target.value }))} className="mt-1" /></div>
            <div><Label>My name</Label><Input value={custodyEditor.parentName} onChange={(e) => setCustodyEditor((current) => ({ ...current, parentName: e.target.value }))} className="mt-1" /></div>
            <div><Label>My email</Label><Input value={custodyEditor.parentEmail} onChange={(e) => setCustodyEditor((current) => ({ ...current, parentEmail: e.target.value }))} className="mt-1" /></div>
            <div><ColorPicker label="My custody color" value={custodyEditor.parentColor} onChange={(color) => setCustodyEditor((current) => ({ ...current, parentColor: color }))} /></div>
            <div><ColorPicker label="Co-parent custody color" value={custodyEditor.coparentColor} onChange={(color) => setCustodyEditor((current) => ({ ...current, coparentColor: color }))} /></div>
            <div><Label>Co-parent name</Label><Input value={custodyEditor.coparentName} onChange={(e) => setCustodyEditor((current) => ({ ...current, coparentName: e.target.value }))} className="mt-1" /></div>
            <div><Label>Co-parent email</Label><Input value={custodyEditor.coparentEmail} onChange={(e) => setCustodyEditor((current) => ({ ...current, coparentEmail: e.target.value }))} className="mt-1" /></div>
          </div>
        </Modal>
      )}

      {confirmAction && (
        <ConfirmModal action={confirmAction} saving={saving} onClose={() => setConfirmAction(null)} />
      )}
    </div>
  );
}
      ...
      footer={
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-end">
          ...
          <Button
            type="button"
            disabled={saving}
            onClick={() => action.onConfirm?.(deleteDays)}
            className={danger ? "bg-red-600 hover:bg-red-700" : "bg-amber-600 hover:bg-amber-700"}
          >
            {saving ? "Working..." : action.confirmText || "Confirm"}
          </Button>
        </div>
      }
    >
      {action.showDeleteDaysOption && (
        <label className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">
          <input type="checkbox" checked={deleteDays} onChange={(e) => setDeleteDays(e.target.checked)} className="mt-1" />
          Also delete custody schedule days linked to this group.
        </label>
      )}
    </Modal>
  );
