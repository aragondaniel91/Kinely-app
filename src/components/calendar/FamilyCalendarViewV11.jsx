...
function assignClusterColumns(cluster) {
  if (cluster.length === 1) {
    return [{ ...cluster[0], columnIndex: 0, columnCount: 1 }];
  }
...
function buildTimedLayout(events, viewMode) {
  ...
  if (viewMode === "week") {
    ...
    byStart.forEach((group, key) => {
      ...
      const hidden = ordered.slice(WEEK_MAX_VISIBLE_PER_START_TIME);
      hidden.forEach((item) => {
        item.hidden = true;
      });
      if (hidden.length) {
        hiddenGroups.push({ key, top: ordered[0].top, events: hidden.map((item) => item.event) });
      }
    });
  }
  ...
  buildOverlapClusters(visible).forEach((cluster) => {
    assignClusterColumns(cluster).forEach((item) => {
      ...
      layoutMap.set(item.event.id, {
        ...
        right: "auto",
        width: `calc(${width}% - ${item.columnCount === 1 ? "1rem" : "0.7rem"})`,
...
function ToolbarButton({ children, active, onClick }) {
  return (
    <button type="button" onClick={onClick} className={cn("h-10 min-w-[76px] border-r border-slate-200 px-4 text-sm font-bold transition last:border-r-0", active ? "bg-blue-600 text-white shadow-sm" : "bg-white text-slate-600 hover:bg-slate-50")}>{children}</button>
  );
}

function FilterButton({ icon: Icon, label, selectedLabel, onClick }) {
  return (
    <button type="button" onClick={onClick} className="flex h-10 min-w-[160px] shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-left text-sm font-bold text-slate-600 shadow-sm transition hover:bg-slate-50 active:scale-[0.98]">
          ...
          {options.map((option) => {
            ...
            return (
              <button key={option.value} type="button" onClick={() => { isCategory ? onCategoryChange(option.value) : onPersonChange(option.value); onClose(); }} className={cn("flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left", active ? "bg-blue-600 text-white shadow-sm" : "text-slate-700 hover:bg-slate-50")}>
                {isCategory ? <span className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-lg", active ? "bg-white/20" : "bg-blue-50")}>{option.emoji}</span> : <PersonDot color={option.color} size="h-9 w-9" />}
...
function MonthYearPicker({ anchorDate, setAnchorDate }) {
  ...
  return (
    <div className="relative z-30">
      <button type="button" onClick={() => { setDisplayYear(anchorDate.getFullYear()); setOpen((current) => !current); }} className="inline-flex items-center gap-2 rounded-xl px-1 text-2xl font-bold text-slate-900 hover:bg-slate-50">{format(anchorDate, "MMMM yyyy")}<ChevronRight className="h-4 w-4 rotate-90 text-slate-500" /></button>
      {open && (
        <div className="absolute left-0 top-11 w-[320px] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl">
          ...
          <div className="grid grid-cols-3 gap-2">{months.map((month, index) => { const active = anchorDate.getFullYear() === displayYear && anchorDate.getMonth() === index; return <button key={month} type="button" onClick={() => { setAnchorDate(new Date(displayYear, index, 1)); setOpen(false); }} className={cn("rounded-2xl px-3 py-3 text-sm font-extrabold", active ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-600 hover:bg-blue-50 hover:text-blue-700")}>{month}</button>; })}</div>
        </div>
      )}
    </div>
  );
...
function FamilyMemberChips({ people, activePerson, onSelectPerson }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      {people.map((person) => { const active = activePerson === person.value; return <button key={person.value} type="button" onClick={() => onSelectPerson(active ? "all" : person.value)} className={cn("flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-semibold transition", active ? "border-blue-600 bg-blue-600 text-white shadow-sm" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50")}><PersonDot color={person.color} /><span>{person.label}</span></button>; })}
    </div>
  );
}
...
function AllDayEvent({ event, profile, onSelect, compact = false, dadName, momName }) {
  ...
  return (
    <button type="button" onClick={(e) => { e.stopPropagation(); onSelect(event, e.currentTarget.getBoundingClientRect()); }} className={cn("w-full rounded-lg border px-2 text-left shadow-sm transition hover:shadow-md", compact ? "py-0.5" : "py-1", colors.bg, colors.border)}>
      <div className="flex items-center justify-between gap-2"><p className="truncate text-[10px] font-extrabold text-slate-900">{event.title}</p><PersonDot color={colorId} size="h-4 w-4" /></div>
...
function EventBlock({ event, profile, selected, onSelect, dadName, momName, layoutStyle }) {
  ...
  return (
    <button type="button" onClick={(e) => { e.stopPropagation(); onSelect(event, e.currentTarget.getBoundingClientRect()); }} className={cn("absolute overflow-hidden rounded-xl border p-2 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md", colors.bg, colors.border, selected && "ring-2", selected && colors.ring)} style={style} title={event.title}>
      <span className={cn("absolute left-0 top-0 h-full w-1.5", colors.stripe)} />
      <div className="pl-1">
        <div className="flex items-start justify-between gap-2"><p className="line-clamp-2 text-[11px] font-extrabold leading-tight text-slate-900 md:text-xs">{event.title}</p><PersonDot color={colorId} size="h-4 w-4" /></div>
    ...
    <div className={cn("relative overflow-x-auto rounded-b-[2rem] bg-white", isSingleDay && "flex justify-center")}>
      <div className={cn(isSingleDay ? "w-full max-w-[760px]" : "min-w-[980px]")}>
        ...
        <div className="grid" style={{ gridTemplateColumns, height: ALL_DAY_HEIGHT + HOURS.length * HOUR_HEIGHT }}>
          <div className="relative border-r border-slate-200 bg-white"><div className="flex items-center justify-center border-b border-slate-200 text-xs font-semibold text-slate-500" style={{ height: ALL_DAY_HEIGHT }}>All-day</div>{HOURS.map((hour) => <div key={hour} className="border-b border-slate-100 pr-2 pt-2 text-right text-sm font-semibold text-slate-500" style={{ height: HOUR_HEIGHT }}>{format(new Date(2026, 0, 1, hour), "h a")}</div>)}</div>
          {days.map((day) => {
            ...
            return (
              <div key={day.toISOString()} className={cn("relative border-r border-slate-200 last:border-r-0", isToday(day) && "bg-blue-50/30")} onClick={() => onAdd(day)}>
                <div className="space-y-1 overflow-hidden border-b border-slate-200 p-2" style={{ height: ALL_DAY_HEIGHT }}>{visibleAllDay.map((event) => <AllDayEvent key={event.id} event={event} profile={profile} onSelect={onSelect} compact dadName={dadName} momName={momName} />)}<MoreAllDayButton count={hiddenAllDay.length} hiddenEvents={hiddenAllDay} onOpen={(hiddenEvents, rect) => onOpenOverflow(hiddenEvents, rect)} /></div>
...
function OverflowPopover({ overflow, profile, dadName, momName, onClose, onSelect }) {
  ...
  return (
    <div className="fixed z-[96] w-[min(340px,calc(100vw-2rem))] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl" style={{ left: overflow.panel.left, top: overflow.panel.top }}>
...
export default function FamilyCalendarViewV11({ viewMode = "week", setViewMode }) {
    ...
    <div className="mx-auto max-w-none overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2"><button type="button" aria-label="Previous" onClick={handlePrevious} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => setAnchorDate(new Date())} className="h-10 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 hover:bg-slate-50">Today</button><button type="button" aria-label="Next" onClick={handleNext} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"><ChevronRight className="h-5 w-5" /></button><MonthYearPicker anchorDate={anchorDate} setAnchorDate={setAnchorDate} /></div>
          <div className="overflow-hidden rounded-xl border border-slate-200"><ToolbarButton active={viewMode === "month"} onClick={() => setViewMode?.("month")}>Month</ToolbarButton><ToolbarButton active={viewMode === "week"} onClick={() => setViewMode?.("week")}>Week</ToolbarButton><ToolbarButton active={viewMode === "day"} onClick={() => setViewMode?.("day")}>Day</ToolbarButton></div>
        </div>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-slate-500">{eventSummary}</p><div className="flex flex-wrap gap-2"><FilterButton icon={UsersRound} label="Person" selectedLabel={selectedPersonLabel} onClick={() => setOpenFilter("person")} /><FilterButton icon={Tags} label="Category" selectedLabel={selectedCategoryLabel} onClick={() => setOpenFilter("category")} /><button type="button" onClick={() => setAddDate(new Date(anchorDate))} className="h-10 rounded-xl bg-blue-600 px-4 text-sm font-black text-white shadow-sm hover:bg-blue-700">Add Event</button></div></div>
        <div className="mt-3"><FamilyMemberChips people={personOptions} activePerson={personFilter} onSelectPerson={setPersonFilter} /></div>
      </div>
