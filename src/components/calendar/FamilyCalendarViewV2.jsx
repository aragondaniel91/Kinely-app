...
function normalizeEvent(docSnap) {
  ...
  return {
    ...
    isAllDay: Boolean(data.isAllDay) || (!data.startTime && !data.endTime),
        ...
        <div className="grid" style={{ gridTemplateColumns, height: ALL_DAY_HEIGHT + HOURS.length * HOUR_HEIGHT }}>
          ...
          {days.map((day) => (
            <div key={day.toISOString()} className={cn("relative border-r border-slate-200 last:border-r-0", isToday(day) && "bg-blue-50/30")} onClick={() => onAdd(day)}>
              <div className="space-y-1 border-b border-slate-200 p-2" style={{ height: ALL_DAY_HEIGHT }}>{dayEvents(day, allDayEvents).slice(0, 1).map((event) => <AllDayEvent key={event.id} event={event} onSelect={onSelect} fallbackChildName={fallbackChildName} />)}</div>
...
function SelectedEventPopover({ selected, onClose, onEdit, onDelete, fallbackChildName }) {
  ...
  const key = personKey(event);
  const colors = personColors[key] || personColors.all;
  const timeText = event.startTime ? `${displayTime(event.startTime)}${event.endTime ? ` – ${displayTime(event.endTime)}` : ""}` : "All day";
...
export default function FamilyCalendarViewV2({ activeCalendar = "family", setActiveCalendar, viewMode = "week", setViewMode }) {
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      const data = snap.docs.map(normalizeEvent).filter((event) => event.date >= dateRange.startKey && event.date <= dateRange.endKey).sort((a, b) => (a.date || "").localeCompare(b.date || "") || (a.startTime || "").localeCompare(b.startTime || ""));
      setEvents(data);
    } catch (error) {
