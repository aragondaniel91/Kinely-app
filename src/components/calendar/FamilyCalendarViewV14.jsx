...
const MAX_WEEK_VISIBLE_PER_HOUR = 3;
...
function eventPosition(event) {
  ...
  return { top: Math.max(ALL_DAY_HEIGHT + 4, top), height, start, end, bucket: Math.floor(start / 60) * 60 };
}
...
function buildDayLayout(events) {
  ...
  return { visibleEvents: items.map((item) => item.event), hiddenGroups: [], layoutMap };
}

function buildWeekHourBucketLayout(events) {
  const items = events
    .map((event, originalIndex) => {
      const position = eventPosition(event);
      if (!position) return null;
  ...
  groups.forEach((group, bucket) => {
    const ordered = [...group].sort((a, b) => a.start - b.start || b.end - a.end || a.originalIndex - b.originalIndex);
    const visibleItems = ordered.slice(0, MAX_WEEK_VISIBLE_PER_HOUR);
    const hiddenItems = ordered.slice(MAX_WEEK_VISIBLE_PER_HOUR);
    const width = 100 / Math.max(1, visibleItems.length);
    ...
    visibleItems.forEach((item, index) => {
      ...
      layoutMap.set(item.event.id, {
        top: item.top,
        height: item.height,
        left: `calc(${width * index}% + 0.5rem)`,
        right: "auto",
        width: `calc(${width}% - ${visibleItems.length === 1 ? "1rem" : "0.7rem"})`,
        zIndex: 40 + index,
      });
    });
    ...
    if (hiddenItems.length) {
      hiddenGroups.push({
        key: `hour:${bucket}`,
        top: ALL_DAY_HEIGHT + ((bucket - DAY_START_MINUTES) / 60) * HOUR_HEIGHT + 8,
...
function buildTimedLayout(events, viewMode) {
  return viewMode === "day" ? buildDayLayout(events) : buildWeekHourBucketLayout(events);
}
...
function OverflowPopover({ overflow, profile, dadName, momName, onClose, onSelect }) {
  ...
  return (
    <div className="fixed z-[96] w-[min(380px,calc(100vw-2rem))] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl" style={{ left: overflow.panel.left, top: overflow.panel.top }}>
      <div className="mb-3 flex items-center justify-between"><div><p className="text-sm font-black text-slate-900">More events</p><p className="text-xs font-semibold text-slate-400">Same hour · tap one to open.</p></div><button type="button" onClick={onClose} className="rounded-full px-2 py-1 text-sm font-bold text-slate-400 hover:bg-slate-100 hover:text-slate-700">×</button></div>
...
export default function FamilyCalendarViewV14({ viewMode = "week", setViewMode }) {
