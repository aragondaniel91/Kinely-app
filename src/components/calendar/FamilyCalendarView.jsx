...
function MonthYearPicker({ anchorDate, setAnchorDate }) {
  ...
  const selectMonth = (monthIndex) => {
    setAnchorDate(new Date(displayYear, monthIndex, 1));
    setOpen(false);
  };
  ...
  return (
    <div className="relative z-30">
      <button
        ...
        className="inline-flex items-center gap-2 rounded-xl px-1 text-2xl font-bold text-slate-900 hover:bg-slate-50"
      >
...
function SelectedEventPopover({ event, onClose, onEdit, onDelete, fallbackChildName }) {
  ...
  return (
    <div className="fixed inset-x-4 bottom-24 z-[40] mx-auto max-w-sm rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl lg:absolute lg:bottom-auto lg:left-1/2 lg:top-[58%] lg:-translate-x-1/2">
