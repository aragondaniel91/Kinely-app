...
function FilterButton({ icon: Icon, label, selectedLabel, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-10 min-w-[160px] shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-left text-sm font-bold text-slate-600 shadow-sm transition hover:bg-slate-50 active:scale-[0.98]"
    >
        ...
        return (
          <button
            ...
            className={cn(
              ...
              active ? "border-blue-600 bg-blue-600 text-white shadow-sm" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
            )}
          >
...
export default function FamilyCalendarViewV9({ viewMode = "week", setViewMode }) {
    ...
    <div className="min-h-full bg-[#f8fbff] p-2 md:p-4">
      <div className="mx-auto max-w-none overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-white px-4 py-4 md:px-8">
          <div className="flex items-start justify-between gap-4 pr-48 md:pr-72">
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-sm"><Sparkles className="h-5 w-5" /></div>
              <div className="min-w-0"><p className="truncate text-xl font-black text-slate-950">Family Wall</p><p className="truncate text-xs font-semibold text-slate-400">Family Calendar</p></div>
            </div>
            <div className="ml-auto inline-flex shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-3 pr-48 md:pr-72">
            ...
            <button type="button" className="flex h-10 shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-600 shadow-sm hover:bg-slate-50">
              <RefreshCw className="h-4 w-4 text-blue-600" /> Sync with external calendar
            </button>
          </div>
        ...
        <div className="border-b border-slate-200 bg-white px-4 py-2 md:px-8">
          <p className="text-left text-sm font-black text-slate-500">{eventSummary}</p>
        </div>

        {loading ? <div className="p-8 text-center text-sm font-bold text-slate-400">Loading family events...</div> : viewMode === "month" ? <MonthView monthDate={anchorDate} events={filteredEvents} profile={profile} onAdd={setAddDate} onSelect={handleSelectEvent} /> : <WeekGrid days={visibleDays} events={filteredEvents} profile={profile} onAdd={setAddDate} onSelect={handleSelectEvent} onOpenAllDayMore={handleOpenAllDayMore} selectedEvent={selectedEvent?.event} dadName={dadName} momName={momName} />}
      </div>

      <button type="button" onClick={() => setAddDate(anchorDate)} className="fixed bottom-24 right-6 z-50 flex h-16 w-16 items-center justify-center rounded-full bg-blue-600 text-white shadow-2xl transition hover:scale-105 hover:bg-blue-700 md:bottom-8 md:right-8" aria-label="Add event"><Plus className="h-7 w-7" /></button>

      {allDayOverflow && <div className="fixed z-[100] w-[min(340px,calc(100vw-2rem))] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl" style={{ left: allDayOverflow.panel.left, top: allDayOverflow.panel.top }}><div className="mb-3 flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-950">All-day events</p><p className="text-xs font-semibold text-slate-400">{allDayOverflow.dateLabel}</p></div><button type="button" onClick={() => setAllDayOverflow(null)} className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-50 hover:text-slate-700"><X className="h-4 w-4" /></button></div><div className="space-y-2">{allDayOverflow.events.map((event) => <AllDayEvent key={event.id} event={event} profile={profile} onSelect={handleSelectEvent} dadName={dadName} momName={momName} />)}</div></div>}
      ...
      {(addDate || editEvent) && <AddFamilyEventDialog date={addDate || new Date(`${editEvent?.date || format(new Date(), "yyyy-MM-dd")}T00:00:00`)} editEvent={editEvent} onClose={() => { setAddDate(null); setEditEvent(null); }} onSuccess={() => { setAddDate(null); setEditEvent(null); loadEvents(); }} />}
