...
function FilterSelect({ icon: Icon, label, value, onChange, children }) {
  return (
    <label className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-600 shadow-sm">
      <Icon className="h-4 w-4 text-slate-400" />
      <span>{label}</span>
      <select
        value={value}
        onChange={(event) => onChange?.(event.target.value)}
        className="min-w-[82px] bg-transparent text-xs font-black text-slate-400 outline-none"
      >
        {children}
      </select>
    </label>
...
export default function FamilyCalendarPlannerHeader({
  ...
  return (
    <header className="bg-white px-6 pt-5">
      ...
      <div className="mt-6 flex items-center justify-between border-t border-slate-100 py-3">
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onToday} className="rounded-xl border border-slate-200 bg-white px-6 py-2 text-sm font-black text-slate-700 shadow-sm hover:bg-slate-50">
            Today
          </button>
        ...
        <div className="flex overflow-hidden rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
          {["day", "week", "month"].map((mode) => (
            <button
              ...
              className={cn(
                "rounded-lg px-5 py-2 text-sm font-black capitalize transition",
