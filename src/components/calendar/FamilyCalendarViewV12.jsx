...
function buildTimedLayout(events, viewMode) {
  ...
  if (viewMode === "week") {
    ...
    byStart.forEach((group, key) => {
      ...
      const hidden = ordered.slice(MAX_WEEK_VISIBLE_PER_START);
      hidden.forEach((item) => {
        item.hidden = true;
      });
      mergeHiddenGroup(hiddenGroups, `start:${key}`, ordered[0].top, hidden.map((item) => item.event));
    });
    ...
    while (changed) {
      ...
      buildOverlapClusters(visibleNow).forEach((cluster) => {
        ...
        const hidden = ordered.slice(MAX_WEEK_VISIBLE_PER_CLUSTER);
        hidden.forEach((item) => {
          if (!item.hidden) {
            item.hidden = true;
            changed = true;
        ...
        mergeHiddenGroup(hiddenGroups, `cluster:${ordered[0].start}`, ordered[0].top, hidden.map((item) => item.event));
      });
    }
  }
...
export default function FamilyCalendarViewV12({ viewMode = "week", setViewMode }) {
  ...
  return (
    <div className="mx-auto max-w-none overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
      <div className="family-calendar-internal-toolbar hidden border-b border-slate-200 bg-white p-4">
        <p>{eventSummary}</p>
        <button type="button" aria-label="Previous" onClick={handlePrevious}><ChevronLeft /></button>
        <button type="button" onClick={() => setAnchorDate(new Date())}>Today</button>
        <button type="button" aria-label="Next" onClick={handleNext}><ChevronRight /></button>
        ...
        <FamilyMemberChips people={personOptions} activePerson={personFilter} onSelectPerson={setPersonFilter} />
      </div>
