...
const WEEK_MAX_VISIBLE_PER_START = 3;
...
function eventPosition(event) {
  ...
  return { top: Math.max(ALL_DAY_HEIGHT + 4, top), height, start, end: end && end > start ? end : start + 45 };
}
...
function assignColumns(items) {
  const sorted = [...items].sort((a, b) => a.start - b.start || a.end - b.end || a.originalIndex - b.originalIndex);
  const columns = [];
  ...
  sorted.forEach((item) => {
    const overlappingIndexes = sorted
      .filter((other) => other === item || eventsOverlap(item, other))
      .map((other) => other.columnIndex);
    columnCount = Math.max(columnCount, Math.max(...overlappingIndexes, 0) + 1);
  });

  return { items: sorted, columnCount };
}
...
function buildTimedLayout(events, viewMode) {
  ...
  if (viewMode === "week") {
    ...
    byStart.forEach((group, key) => {
      ...
      const hidden = ordered.slice(WEEK_MAX_VISIBLE_PER_START);
      hidden.forEach((item) => {
        item.hidden = true;
      });
      if (hidden.length) {
  ...
  const { items: positioned, columnCount } = assignColumns(visible);
...
function ToolbarButton({ children, active, onClick }) {
  return (
    <button type="button" onClick={onClick} className={cn("h-10 min-w-[76px] border-r border-slate-200 px-4 text-sm font-bold transition last:border-r-0", active ? "bg-blue-600 text-white shadow-sm" : "bg-white text-slate-600 hover:bg-slate-50")}>
      {children}
    </button>
  );
}
...
function WeekGrid({ days, events, profile, viewMode, onAdd, onSelect, onOpenAllDayMore, onOpenTimedMore, selectedEvent, dadName, momName }) {
          ...
          {days.map((day) => {
            ...
            return (
              <div key={day.toISOString()} className={cn("relative border-r border-slate-200 last:border-r-0", isToday(day) && "bg-blue-50/30")} onClick={() => onAdd(day)}>
                ...
                {viewMode === "week" && hiddenGroups.map((group) => <button key={group.key} type="button" onClick={(e) => { e.stopPropagation(); onOpenTimedMore(group.events, e.currentTarget.getBoundingClientRect()); }} className="absolute right-2 z-50 rounded-full border border-slate-300 bg-white px-2 py-1 text-xs font-black text-slate-600 shadow-sm hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700" style={{ top: Math.max(ALL_DAY_HEIGHT + 8, group.top + 8) }}>+{group.events.length}</button>)}
              </div>
            );
          })}
        </div>
...
export default function FamilyCalendarViewV10({ viewMode = "week", setViewMode }) {
      ...
      <div className="border-b border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <button type="button" aria-label="Previous" onClick={handlePrevious} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"><ChevronLeft className="h-5 w-5" /></button>
            <button type="button" onClick={() => setAnchorDate(new Date())} className="h-10 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 hover:bg-slate-50">Today</button>
            <button type="button" aria-label="Next" onClick={handleNext} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"><ChevronRight className="h-5 w-5" /></button>
