...
function itemIsActiveInHour(item, bucket) {
  return item.start < bucket + 60 && item.end > bucket;
}
...
function buildTimedLayout(events, viewMode) {
  ...
  if (viewMode === "day") {
    return { visibleEvents: items.map((item) => item.event), hiddenGroups: [], layoutMap: buildColumnLayout(items) };
  }
  ...
  return { visibleEvents: visibleItems.map((item) => item.event), hiddenGroups, layoutMap: buildColumnLayout(visibleItems) };
}
...
function WeekGrid({ days, events, profile, viewMode, onAdd, onSelect, onOpenOverflow, selectedEvent, dadName, momName }) {
  ...
  const dayEvents = (day, list) => list.filter((event) => event.date === dateKey(day));
...
function MonthView({ monthDate, events, profile, onAdd, onSelect, onOpenDay }) {
          ...
          return (
            <button key={day.toISOString()} type="button" onClick={() => onAdd(day)} className={cn("min-h-[128px] rounded-2xl border border-slate-200 bg-white p-2 text-left transition hover:border-blue-200 hover:bg-blue-50/30", isToday(day) && "ring-2 ring-blue-400", !isSameMonth(day, monthDate) && "opacity-45")}>
              ...
              <div className="space-y-1">
                ...
                {list.length > 3 && <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); onOpenDay(day); }} className="w-full rounded-lg border border-dashed border-slate-300 bg-slate-50 px-2 py-1 text-left text-[10px] font-black text-slate-500 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">+{list.length - 3} more</button>}
              </div>
            </button>
          );
        })}
...
function SelectedEventPopover({ selected, profile, dadName, momName, onClose, onEdit, onDelete }) {
  ...
  return (
    <div className="fixed z-[95] rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl" style={{ left: panel.left, top: panel.top, width: panel.mode === "sheet" ? "calc(100vw - 2rem)" : PANEL_WIDTH, maxWidth: "calc(100vw - 2rem)" }}>
      <div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-extrabold text-slate-950">{event.title}</h3><p className="mt-1 text-sm font-medium text-slate-600">{event.date ? format(dateFromKey(event.date), "EEE, MMM d") : ""} · {timeText}</p><p className="mt-1 text-sm text-slate-500">{categoryEmoji(event)} {categoryLabel(event)}</p></div><button type="button" onClick={onClose} className="rounded-full px-2 py-1 text-sm font-bold text-slate-400 hover:bg-slate-100 hover:text-slate-700">×</button></div>
...
export default function FamilyCalendarViewV16({ viewMode = "week", setViewMode }) {
  ...
  const viewEvents = useMemo(() => {
    if (viewMode === "day") return filteredEvents.filter((event) => event.date === dateKey(anchorDate));
    if (viewMode === "month") return filteredEvents.filter((event) => event.date && isSameMonth(dateFromKey(event.date), anchorDate));
    const visible = new Set(visibleDays.map(dateKey));
    return filteredEvents.filter((event) => visible.has(event.date));
  }, [anchorDate, filteredEvents, viewMode, visibleDays]);

  const eventSummary = useMemo(() => {
    const label = viewEvents.length === 1 ? "event" : "events";
    if (viewMode === "day") return `${viewEvents.length} ${label} | ${format(anchorDate, "MMM d")}`;
    if (viewMode === "month") return `${viewEvents.length} ${label} | ${format(anchorDate, "MMMM yyyy")}`;
    const first = visibleDays[0] || anchorDate;
    const last = visibleDays[visibleDays.length - 1] || anchorDate;
    return `${viewEvents.length} ${label} | ${format(first, "MMM d")} - ${format(last, "MMM d")}`;
  }, [anchorDate, viewEvents.length, viewMode, visibleDays]);
  ...
  return (
    <div className="mx-auto max-w-none overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
      ...
      {loading ? <div className="p-10 text-center text-sm font-bold text-slate-400">Loading calendar...</div> : viewMode === "month" ? <MonthView monthDate={anchorDate} events={filteredEvents} profile={profile} onAdd={setAddDate} onSelect={handleSelectEvent} onOpenDay={handleOpenDay} /> : <WeekGrid days={visibleDays} events={filteredEvents} profile={profile} viewMode={viewMode} onAdd={setAddDate} onSelect={handleSelectEvent} onOpenOverflow={handleOpenOverflow} selectedEvent={selectedEvent?.event} dadName={dadName} momName={momName} />}
