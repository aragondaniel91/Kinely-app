...
function AllDayEvent({ event, profile, onSelect, compact = false, dadName, momName }) {
  ...
  return (
    <button
      ...
      className={cn(
        "w-full rounded-lg border px-2 text-left shadow-sm transition hover:shadow-md",
        compact ? "py-0.5" : "py-1",
        colors.bg,
        colors.border
      )}
    >
      ...
      {!compact && (
        <p className="truncate text-[9px] text-slate-600">
          {categoryLabel(event)} · {eventPersonLabel(event, dadName, momName)}
        </p>
      )}
    </button>
  );
              ...
              <div key={day.toISOString()} className={cn("relative border-r border-slate-200 last:border-r-0", isToday(day) && "bg-blue-50/30")} onClick={() => onAdd(day)}>
                <div className="space-y-1 overflow-hidden border-b border-slate-200 p-2" style={{ height: ALL_DAY_HEIGHT }}>
                  {visibleAllDay.map((event) => (
                    <AllDayEvent key={event.id} event={event} profile={profile} onSelect={onSelect} compact dadName={dadName} momName={momName} />
                  ))}
                ...
                {dayEvents(day, timedEvents).map((event) => (
                  <EventBlock key={event.id} event={event} profile={profile} selected={selectedEvent?.id === event.id} onSelect={onSelect} dadName={dadName} momName={momName} />
                ))}
              </div>
            );
          })}
  ...
  return (
    <div className="rounded-b-[2rem] bg-white p-3">
      <div className="grid grid-cols-7 gap-2 pb-2">
        {labels.map((label) => <div key={label} className="text-center text-xs font-extrabold uppercase tracking-wide text-slate-500">{label}</div>)}
      </div>
...
function SelectedEventPopover({ selected, profile, dadName, momName, onClose, onEdit, onDelete }) {
  ...
  return (
    <div
      ...
      style={{ left: panel.left, top: panel.top, width: panel.mode === "sheet" ? "calc(100vw - 2rem)" : PANEL_WIDTH, maxWidth: "calc(100vw - 2rem)" }}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-extrabold text-slate-950">{event.title}</h3>
          <p className="mt-1 text-sm font-medium text-slate-600">
            {event.date ? format(new Date(`${event.date}T00:00:00`), "EEE, MMM d") : ""} · {timeText}
          </p>
          <p className="mt-1 text-sm text-slate-500">{categoryEmoji(event)} {categoryLabel(event)}</p>
        </div>
      ...
      <div className="mt-4 flex items-center gap-2">
        <PersonDot color={colorId} size="h-8 w-8" />
        <span className={cn("rounded-full border px-2 py-1 text-xs font-black", color.bg, color.border, color.text)}>{eventPersonLabel(event, dadName, momName)}</span>
        {event.googleCalendarEventId && <span className="ml-auto rounded-full bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700">Google synced</span>}
      </div>
      <div className="mt-4 space-y-2 rounded-2xl bg-slate-50 p-3 text-sm text-slate-600">
        {event.location && <p className="flex gap-2"><MapPin className="mt-0.5 h-4 w-4 shrink-0" />{event.location}</p>}
        ...
        return (
          <button
            ...
            className={cn(
              ...
              active ? "border-blue-600 bg-blue-600 text-white shadow-sm" : "border-transparent text-slate-700 hover:border-slate-200 hover:bg-slate-50"
            )}
          >
...
function CompactFilterButton({ label, selectedLabel, onClick }) {
  return (
      ...
      <div className="relative flex-1 bg-white p-8">
        <div className="mx-auto flex min-h-[420px] max-w-3xl items-center justify-center rounded-[2rem] border border-slate-200 bg-slate-50/70 p-8 text-center">
          <div className="max-w-lg">
            <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl bg-blue-50 text-blue-600"><CalendarDays className="h-8 w-8" /></div>
            <h2 className="text-3xl font-black text-slate-950">{activeCalendar === "custody" ? "Custody Calendar" : "All Calendar"}</h2>
            <p className="mt-3 text-base font-semibold text-slate-500">Use the selector on the Calendar page to open the dedicated custody view.</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="mx-auto max-w-none overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-white px-4 py-4 md:px-8">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-semibold text-slate-400">Profile colors are active</p>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select value={activeCalendar} onChange={(e) => setActiveCalendar?.(e.target.value)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-600 shadow-sm">
                <option value="family">Family Calendar</option>
                <option value="custody">Custody Calendar</option>
          ...
          <div className="mt-3 flex flex-wrap gap-2 md:hidden">
            <CompactFilterButton label="Person" selectedLabel={selectedPersonLabel} onClick={() => setOpenFilter("person")} />
            <CompactFilterButton label="Category" selectedLabel={selectedCategoryLabel} onClick={() => setOpenFilter("category")} />
          </div>

          <div className="mt-3 hidden md:flex md:items-center md:justify-between md:gap-3">
            <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-600 shadow-sm">
              {categoryOptions.map((category) => <option key={category.value} value={category.value}>{category.emoji} {category.label}</option>)}
            </select>
            <p className="text-xs font-semibold text-slate-400">Events use colors from Profile → Families.</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <WeekGrid
            ...
            onOpenAllDayMore={handleOpenAllDayMore}
      ...
      {allDayOverflow && (
        <div className="fixed z-[100] w-[min(340px,calc(100vw-2rem))] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl" style={{ left: allDayOverflow.panel.left, top: allDayOverflow.panel.top }}>
          <div className="mb-3 flex items-start justify-between gap-3">
            <div><p className="text-sm font-black text-slate-950">All-day events</p><p className="text-xs font-semibold text-slate-400">{allDayOverflow.dateLabel}</p></div>
            <button type="button" onClick={() => setAllDayOverflow(null)} className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-50 hover:text-slate-700"><X className="h-4 w-4" /></button>
          </div>
          <div className="space-y-2">
            {allDayOverflow.events.map((event) => <AllDayEvent key={event.id} event={event} profile={profile} onSelect={handleSelectEvent} dadName={dadName} momName={momName} />)}
          </div>
        </div>
      )}
